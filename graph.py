...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        bfs_queue.enqueue(start_index)

        while not bfs_queue.is_empty():
            current_vertex_index = bfs_queue.dequeue()
            current_vertex = self.vertices[current_vertex_index]

            if current_vertex_index not in visited_vertices:
                visited_vertices.add(current_vertex_index)

                if current_vertex.color == initial_color:
                    current_vertex.visit_and_set_color(color)

                for adjacent_index in current_vertex.edges:
                    adjacent_vertex = self.vertices[adjacent_index]

                    if adjacent_index not in visited_vertices and adjacent_vertex.color == initial_color:
                        if adjacent_vertex.color == initial_color:
                            bfs_queue.enqueue(adjacent_index)
    ...
    def dfs(self, start_index, color):
        ...
        dfs_stack.push(start_index)

        while not dfs_stack.is_empty():
            current_vertex_index = dfs_stack.pop()
            current_vertex = self.vertices[current_vertex_index]
